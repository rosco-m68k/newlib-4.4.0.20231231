 ...
 *  call to _set_debug_traps() is necessary in order to allow any breakpoints
 ...
 *  The external function _exceptionHandler() is
...
void _putDebugChar(int);                            /* write a single character      */
int _getDebugChar();                                /* read and return a single char */
void _cleanup_debugger(void);     

Function _exceptionHandler(int, void (*)(void));    /* assign an exception handler */
extern ExceptionHook exceptionHook;                 /* hook variable for errors/exceptions */

void _return_to_super(void);
void _return_to_user(void);
void _catchException(void);
    ...
    retry:
        ...
        if (ch == '#') {
            ...
            if (checksum != xmitcsum) {
                ...
                _putDebugChar('-'); /* failed checksum */
            } else {
                _putDebugChar('+'); /* successful transfer */

                /* if a sequence char is present, reply the sequence ID */
                if (buffer[2] == ':') {
                    _putDebugChar(buffer[0]);
                    _putDebugChar(buffer[1]);
...
static void putpacket(char *buffer) {
    ...
    do {
        ...
        _putDebugChar('#');
        _putDebugChar(hexchars[checksum >> 4]);
        _putDebugChar(hexchars[checksum % 16]);
    ...
    while (1 == 1) {
        ...
        case 'm':
            if (setjmp(remcomEnv) == 0) {
                ...
                _exceptionHandler(2, handle_buserror);
                ...
                _exceptionHandler(2, _catchException);
...
void _set_debug_traps()
{
    ...
    for (exception = 2; exception <= 23; exception++)
        _exceptionHandler(exception, _catchException);

    /* level 7 interrupt              */
    _exceptionHandler(31, _debug_level7);
    ...
    if (oldExceptionHook != _remcomHandler) {
